...
interface CuratedParlay {
  ...
  parlay_type:    'sgp' | 'multi' | 'premium' | 'value' | 'jackpot'
  game_date:      string
...
async function getFeedParlays(): Promise<CuratedParlay[]> {
...
export default async function FeedPage() {
  const parlays = await getFeedParlays()
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold text-white">Parlay Feed</h1>
        <p className="text-white/40 text-sm">Curated parlays posted by the Prizm team</p>
      </div>
